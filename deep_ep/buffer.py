...
class Buffer:
    ...
    def dispatch(self, x: Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]],
                 ...
                 unzip_alignment: int = 0,
                 unzip_chunk_size: int = 0) -> \
        ...
        Returns:
            ...
            event: the event after executing the kernel (valid only if `async_finish` is set).
            unzipped_tokens, unzipped_probs: only appended to the returned tuple when `unzip_alignment > 0`
                ...
                `unzipped_tokens` follows the same type as the input `x`, i.e. a `(tokens, scales)` tuple
                for FP8.
        ...
        if self.runtime.get_num_rdma_ranks() > 1:
            return self.internode_dispatch(x, handle, num_tokens_per_rank, num_tokens_per_rdma_rank, is_token_in_rank,
                                           ...
                                           previous_event, async_finish, allocate_on_comm_stream, unzip_alignment, unzip_chunk_size)

        assert unzip_alignment == 0, 'fused unzip is internode-only'
    ...
    def internode_dispatch(self, x: Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]],
        ...
        if handle is not None:
            ...
            recv_x, recv_x_scales, recv_topk_idx, recv_topk_weights, num_recv_tokens_per_expert_list, \
                ...
                expert_alignment, num_worst_tokens, config, getattr(previous_event, 'event', None), async_finish, allocate_on_comm_stream,
                unzip_alignment, unzip_chunk_size)
